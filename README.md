...
`git/hooks/pre-commit` then runs in every repo on the machine. It first runs the repo's own
`.git/hooks/pre-commit` (which a global `hooksPath` would otherwise hide, and which may lint or re-stage
formatted files), then calls `git/hooks/agent-review.sh` on the diff that will actually land.
...
on a `VERDICT: BLOCK`, and the full review is saved as `agent-review.last.md` inside the repo's git dir
(the hook prints the exact path) so the agent can read and fix it. Commits made by a human are not
reviewed.

| committing agent | family | reviewer tried in order |
| --- | --- | --- |
| Claude Code, opencode | anthropic | `opencode run -m openai/gpt-5.5`, `grok --prompt-file`, `opencode run -m xai/…` |
| grok | xai | `opencode run -m openai/gpt-5.5`, `claude -p --model sonnet`, `opencode run -m anthropic/…` |
| codex | openai | `grok --prompt-file`, `opencode run -m xai/…`, `claude -p --model sonnet` |
