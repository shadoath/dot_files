...
- The color is pinned for this terminal, so the Stop hook won't overwrite it. The pin clears on `reset`, when the terminal exits, on reboot, or after half a day with no session using it. `/clear` and `resume` keep it.
