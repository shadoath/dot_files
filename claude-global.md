...
- **Always type the level as the FIRST token: `/code-review medium`, then the target.** The level is sticky (`codeReviewLastEffort` in `~/.claude.json`) and a level in any other position does not parse — `/code-review 40 medium` reads the whole string as a target and silently reuses whatever ran last. A single stray `high` therefore taxes every later bare `/code-review`.
- **Don't run `/code-review` on trivial diffs at all.** Markdown, comments, config one-liners, and anything under ~20 lines of prose: read it yourself and skip the skill. Review subagents were 48% of a recent 24h spend, much of it reviewing documentation edits.
