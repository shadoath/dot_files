...
- **When Codex is offline, move the review to another model — never skip it.** Codex replying `You have reached your Codex usage limits for code reviews`, or saying nothing within the global 10-minute cap, means the outside opinion is unavailable, not waived. Run the same rulebook on OpenAI's sol: `{ echo "Review this PR diff; report Major findings with file:line, then a final VERDICT: APPROVE|BLOCK line."; gh pr diff <N>; } | opencode run --pure -m openai/gpt-5.6-sol --agent review`. Work its major findings, name the reviewer that actually ran in the PR, and re-post `@codex review` if the quota returns before merge. (`AGENT_REVIEW_MODEL=openai/gpt-5.6-sol` forces the same model in the pre-commit hook.)
